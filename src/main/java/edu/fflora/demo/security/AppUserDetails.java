package edu.fflora.demo.security;

import edu.fflora.demo.models.User;
import org.springframework.security.core.GrantedAuthority;
import org.springframework.security.core.authority.SimpleGrantedAuthority;
import org.springframework.security.core.userdetails.UserDetails;

import java.util.Collection;
import java.util.List;

public class AppUserDetails implements UserDetails {
    private User user;

    public AppUserDetails(User user) {
        this.user = user;
    }

//    @Override
//    public Collection<? extends GrantedAuthority> getAuthorities() {
//        if ("Client".equals(user.getRole().getDesignation())) {
//            return List.of(new SimpleGrantedAuthority("ADMINISTRATEUR"));
//        } else if ("Ouvrier".equals(user.getRole().getDesignation())) {
//            return List.of(new SimpleGrantedAuthority("ROLE_OUVRIER"));
//        } else {
//            return List.of(new SimpleGrantedAuthority("ROLE_AUTRE"));
//        }
//    }


    @Override
    public Collection<? extends GrantedAuthority> getAuthorities() {
        if ("Client".equals(user.getRole().getDesignation())) {
            return List.of(new SimpleGrantedAuthority("ROLE_CHEF"));
        } else if ("Ouvrier".equals(user.getRole().getDesignation())) {
            return List.of(new SimpleGrantedAuthority("ROLE_OUVRIER"));
        } else if ("Autre".equals(user.getRole().getDesignation())) {
            return List.of(new SimpleGrantedAuthority("ROLE_AUTRE"));
        } else {
            return List.of(new SimpleGrantedAuthority("ROLE_ADMINISTRATEUR"));
        }
    }
    @Override
    public String getPassword() {
        return user.getPassword();
    }

    @Override
    public String getUsername() {
        return user.getPseudo();
    }

    @Override
    public boolean isAccountNonExpired() {
        return true;
    }

    @Override
    public boolean isAccountNonLocked() {
        return true;
    }

    @Override
    public boolean isCredentialsNonExpired() {
        return true;
    }

    @Override
    public boolean isEnabled() {
        return true;
    }
}
